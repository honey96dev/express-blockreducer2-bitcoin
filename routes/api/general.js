...
const volume1Proc = (req, res, next) => {
    ...
    let sql = sprintf("SELECT COUNT(`timestamp`) `cnt` FROM (SELECT V.timestamp FROM `%s_%s` V LEFT JOIN `%s_%s` W ON W.timestamp = V.timestamp LEFT JOIN `%s_%s_%s` I ON I.timestamp = V.timestamp WHERE V.timestamp BETWEEN '%s' AND '%s' ORDER BY `timestamp`) `tmp`" , dbTblName.volume, binSize, dbTblName.vwap, binSize, dbTblName.id0, symbol, binSize, startTime, endTime);
