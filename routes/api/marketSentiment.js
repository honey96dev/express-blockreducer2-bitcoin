...
const oneProc = (req, res, next) => {
  ...
  let sql = sprintf("SELECT * FROM `%s_%s` WHERE `symbol` = '%s' ORDER BY `timestamp` DESC LIMIT 1", dbTblName.id0, binSize, symbol);
...
const collectionProc = (req, res, next) => {
  ...
  let sql = sprintf("SELECT COUNT(`timestamp`) `count` FROM `%s_%s` WHERE `symbol` = '%s' AND `timestamp` BETWEEN '%s' AND '%s';", dbTblName.id0, binSize, symbol, startTime, endTime);
  dbConn.query(sql, null, (error, rows, fields) => {
    if (error) {
      console.error(error);
      res.status(200).send({
